...
namespace Algebra
{
    ...
    public:
        ...
        Matrix& subMatrix(int64_t top, int64_t left, int64_t bottom = -1, int64_t right = -1) &&;
    ...
    private:
        ...
        static void realTransposeInPlace(double* A, const size_t r, const size_t c, const size_t lda);
        static Matrix householderReflect(const Matrix& u);
        void householderReflectSubM(const Matrix& u, size_t r, size_t c);
    ...
    Matrix& Matrix::operator*=(const Matrix &mat)
    {
        ...
        if (row * mat.col <= buff_size)
        {
            ...
            if (this->m == buff)
                ...
                realTransposeInPlace(m, row, col, col);
        }
        else
        {
    ...
    {
        Matrix Q(row, row), R(*this);
        auto min = std::min(row, col);

        for (size_t i = 0; i < min; i++)
        {
            Matrix v = R.subMatrix(i,i, -1,i);
            double norm = v.normSquare();
            v[0] += v[0] < 0 ? -norm : norm;
            v /= v[0];
            R.householderReflectSubM(v, i,i);
            
            double tau_i = 2 / v.normSquare();
        }
    ...
    void Matrix::setSubMatrix(const Matrix &mat, int64_t r, int64_t c)
    {
        ...
        for (size_t i = 0; i < endr; i++)
            std::copy(mat.m + i*mat.col, mat.m + i*mat.col + endc, m + (r+i)*col + r);
    }
    ...
    Matrix Matrix::subMatrix(int64_t t, int64_t l, int64_t b, int64_t r) const &
    {
        ...
        for (size_t i = t; i <= b; i++)
            std::copy(m + i*col + l, m + i*col + r, sm.m);
        return sm;
    }

    Matrix& Matrix::subMatrix(int64_t t, int64_t l, int64_t b, int64_t r) &&
    {
        if (t < 0) t += col;
        if (l < 0) l += col;
        if (b < 0) b += col;
        ...
        if (l >= col || l < 0 || r >= col || r < 0 || t >= row || t < 0 || b >= row || b < 0)
            throw std::out_of_range("Matrix coordinates out of range in Matrix::subMatrix &&");
        size_t ncol = r-l, nrow = b-t;
        if (nrow*ncol <= buff_size)
        {
            if (m == buff)
                for (size_t i = 0; i < nrow; i++)
                    std::copy(m + (t+i)*col + l, m + (t+i)*col + r + 1, buff + i*ncol);
            else
            {
                for (size_t i = 0; i < nrow; i++)
                    std::copy(m + (t+i)*col + l, m + (t+i)*col + r + 1, buff + i*ncol);
        ...
        {
            double* new_buf = (double*)std::aligned_alloc(32, nrow*ncol);
            for (size_t i = 0; i < row; i++)
                std::copy(m + (t+i)*col + l, m + (t+i)*col + r + 1, new_buf + i*ncol);
            std::free(m);
            m = new_buf;
        }
    ...
    {
        const size_t max_r = r & ~(1L);
        const size_t max_c = c & ~(1L);

        #pragma omp parallel for
        for (size_t i = 0; i < max_r; i += 4)
        {
            for (size_t j = 0; j < max_c; j += 4)
    ...
    void Matrix::realTransposeInPlace(double *A, const size_t r, const size_t c, const size_t lda)
    {
        const size_t max_r = r & ~(1L);
        const size_t max_c = c & ~(1L);
        ...
        for (size_t i = 0; i < max_r; i += 4)
        {
            for (size_t j = 0; j < max_c; j += 4)
                for (size_t k = 0; k < 4; k++)
                    for (size_t l = 0; l < 4; l++)
                        std::swap(A[(j + l)*r + (i + k)], A[(i + k)*c + j + l]);

            for (size_t k = 0; k < 4; k++)
                for (size_t j = max_c; j < c; j++)
                    std::swap(A[j*r + (i + k)], A[(i + k)*c + j]);
        }

        for (size_t i = max_r; i < r; i++)
            for (size_t j = 0; j < c; j++)
                std::swap(A[j*r + i], A[i*c + j]);
    }

    Matrix Matrix::householderReflect(const Matrix &u)
    {
        double tau_i = 2 / u.normSquare();
        Matrix ref = (-u/tau_i)*u.T();
        for (size_t i = 0; i < u.getSize(); i++)
            ref.val(i,i) += 1.;
        return ref;
    }

    void Matrix::householderReflectSubM(const Matrix &u, size_t r, size_t c)
    {
